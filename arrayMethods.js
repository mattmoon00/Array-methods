...
// CODE HERE
const totalPopulation = populations.reduce((acc, curElement) => acc + curElement)
...
const addTax = (arr) => {
  let pricesArr = arr.map((e,i,array) => {
    let result = e["price"] + (e.price * e.tax)
    // console.log(result)
    return result
  })
  return pricesArr
...
*/
  ...
  // let bobsTotalArr = purchases.filter(function(owner){
  //   if (owner.owner === "Bob") {
  //     return true
  //   }
  //     return false
  ...
  // console.log(bobsTotalArr)

  // let bobsTotal = bobsTotalArr.forEach((price) => {
  //     let bobsPrice = price.price
  //     let realPrice = d3.sum(bobsTotalArr)
